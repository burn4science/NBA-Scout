...
The data ingestion capability fetches NBA team, player, and per-season statistical data from the `nba_api` library and upserts it into the application database. It is fully configurable via `config.toml`, respects API rate limits, and emits structured logs via `loguru`.
